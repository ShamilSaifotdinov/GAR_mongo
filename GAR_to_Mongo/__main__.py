...
def main(argv):
    parse_xml('D:\\Documents\\АСКОН\\Автоопр\\gar-xml\\', '50')
...
class parse_xml:
    def __init__(self, path, region_code):
        ...
        self.upd = update_field(self.region_code)
    ...
    def parse_addr_obj(self, path):
        ...
        for event, elem in tree:
            ...
            self.region.insert_many(arr)
            self.region.create_index('objectId', unique=True )
            ...
            if elem.tag == 'PARAM' and elem.attrib['CHANGEIDEND'] == "0":
                try:
                    if elem.attrib['TYPEID'] == "6":
                        ...
                        self.upd.add_value(int(elem.attrib['OBJECTID']), 'OKATO', elem.attrib['VALUE'])
                        ...
                        self.upd.add_value(int(elem.attrib['OBJECTID']), 'OKTMO', elem.attrib['VALUE'])
                        ...
                        self.upd.add_value(int(elem.attrib['OBJECTID']), 'CODE', elem.attrib['VALUE'])
                except Exception as e:
                    print("Error:", e)
            if event == "end":
                elem.clear()
        ...
        for event, elem in tree:
            if elem.tag == 'ITEM' and elem.attrib['ISACTIVE'] == "1":
                try:
                    ...
                    self.upd.add_value(int(elem.attrib['OBJECTID']), f'{type}_parentId', int(elem.attrib['PARENTOBJID']))
                except Exception as e:
                    print(e)
